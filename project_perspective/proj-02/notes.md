...
container: list[int] = []
elem: etree.ElementBase = etree.fromstring(b, parser=etree.HTMLParser())
```

> 实际上很多简单的变量根本用不着标注，`MyPy`这样的静态类型检查工具很容易推断出它们的值。你通常更需要标注的是像「某个接受了复杂且没标注的函数的返回值的变量」，和那些「在开始被赋值为空的容器」，还有一些「标注不良好没法自动补全的第三方库」之类的，**推断失败的东西**。
>
> ~~`lxml`说的就是你~~
