...
同理，可以通过类似的方法实现取消下载，只是从阻塞线程[^1]变成了直接退出[^2]而已。

[^1]: `event.wait()`
[^2]: `if event.is_set(): return`
...
具体实现可以借助**`concurrent`标准库 ~~，这不比自己搓线程管理方便？不过还是可以了解了解~~
...
将媒体、合集等来源的输入转换为基本的视频下载流程，并执行。可以使用`tqdm`库来跟踪进度。
