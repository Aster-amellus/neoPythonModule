...
将主体部分缩进一级，将预设的URL改成传入参数，将打印封面URL改成返回封面URL，移除多余的打印语句。将作为常量的headers改写为全大写，放在函数外面。添加`main`函数。

> 若当前模块中的`__name__`变量为`"__main__"`，则说明当前模块是被直接运行的。否则此变量的内容为当前模块的名字。
>
...
> 注意请勿违反国家法律法规，对于滥用造成的任何后果我们都不对此负责。